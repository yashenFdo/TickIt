...
export const FilterBar: React.FC<FilterBarProps> = ({ filters, onFilterChange, onReset }) => {
  const cities = ['All Locations', 'New York, NY', 'Los Angeles, CA', 'London, UK', 'Berlin, DE', 'Las Vegas, NV', 'Ibiza, ES'];

  return (
    <div className="bg-netflix-dark-grey p-4 rounded-md border border-white/5 space-y-4 shadow-md text-xs">
      <div className="flex items-center justify-between border-b border-white/10 pb-3">
        <div className="flex items-center space-x-2 text-netflix-white font-bold">
          ...
          <span className="text-sm">Filter & Sort Events</span>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="space-y-1.5">
          ...
          <select
            ...
            className="w-full bg-netflix-black text-netflix-white py-2 px-3 rounded-md border border-white/10 focus:border-netflix-red focus:outline-none"
          >
            <option value="match">Highest Netflix Match %</option>
