      ...
      <div className="w-full max-w-md bg-netflix-dark-grey text-netflix-white h-full overflow-y-auto shadow-2xl flex flex-col justify-between border-l border-white/10 animate-slideLeft">
        ...
        <div className="p-5 border-b border-white/10 flex items-center justify-between bg-netflix-dark-grey sticky top-0 z-10">
          <div className="flex items-center space-x-2">
            ...
            <div>
              <h2 className="font-extrabold text-lg tracking-tight text-netflix-white">My Wallet & Passes</h2>
        ...
        <div className="p-5 flex-1 space-y-4">
          {tickets.length === 0 ? (
            <div className="text-center py-16 space-y-3">
              ...
              <h3 className="text-base font-bold text-netflix-white">No Active Tickets</h3>
              <p className="text-xs text-netflix-light-grey max-w-xs mx-auto">
                Explore live concerts, comedy shows, and sports events to reserve your seats!
              </p>
            </div>
          ) : (
            tickets.map((ticket) => (
                ...
                <div className="bg-netflix-dark-grey p-3 rounded-md flex items-center justify-between border border-white/5">
                  <div className="flex items-center space-x-2">
                    ...
                    <div className="text-[10px] text-netflix-light-grey">
                      <div className="font-mono text-netflix-white">PASS ID: {ticket.id}</div>
        ...
        {transferTicketId && (
          <div className="absolute inset-0 bg-netflix-black/90 z-30 p-6 flex flex-col justify-center animate-fadeIn space-y-4">
            <div className="flex justify-between items-center border-b border-white/10 pb-3">
              <div className="font-bold text-sm text-netflix-white flex items-center gap-2">
                <Send className="w-4 h-4 text-netflix-red" />
                <span>Transfer Ticket to Friend</span>
              </div>
            ...
            {transferSuccess ? (
              <div className="text-center py-6 space-y-2">
                ...
                <div className="font-bold text-netflix-white text-sm">Ticket Transferred!</div>
                <p className="text-xs text-netflix-light-grey">
                  Entry pass sent to <span className="text-netflix-red font-semibold">{recipientEmail}</span>.
                </p>
              </div>
            ) : (
              <form onSubmit={handleTransfer} className="space-y-4">
                ...
                <button
                  type="submit"
                  className="w-full bg-netflix-red hover:bg-netflix-red/90 text-white font-bold py-2.5 rounded-md text-xs transition-colors"
                >
                  Send Ticket Pass
                </button>
              </form>
        ...
        <div className="p-5 border-t border-white/10 bg-netflix-dark-grey">
          <button
            onClick={onClose}
            className="w-full bg-netflix-red hover:bg-netflix-red/90 text-white font-bold py-2.5 rounded-md text-sm transition-colors cursor-pointer"
          >
            Close Wallet
          </button>
        </div>
